...
if [[ $loginDate == *T* ]]; then
  login="$(date -d $loginDate +"%a %d %b %Y, %k:%M ") $(color $dimInfoColor "» $loginIP")"
else
  # First Login
  login="None"
fi
...
# Stats
label1="$login"
label1="$(color $statsLabelColor "=> Last Login") $(color $bulletColor "···")$(color $statsLabelColor "›") $(color $infoColor "$label1")"
...
uptime="$uptime $(color $dimInfoColor "» $(date -d "@"$(grep btime /proc/stat | cut -d " " -f 2) +"%d-%m-%y %H:%M")")"
